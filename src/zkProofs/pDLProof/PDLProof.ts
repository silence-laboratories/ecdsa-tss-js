...
export default class PDLProof {
	...
	static async prove(
		...
		const compositeDLogProof = await CompositeDLogProof.prove(
			dLogStatement,
			xhi,
			sid,
			pid,
		...
		const pDLwSlackProof = await PDLwSlackProof.prove(
			pDLwSlackWitness,
			pDSwSlackStatement,
			sid,
			pid,
	...
	static async verify(
		...
		if (
			!utils.comparePaillierPublicKey(
				pDSwSlackStatement.ek,
				paillierPublicKey,
			) ||
		...
		const dlogStatement = new DLogStatement(
			pDSwSlackStatement.nTilde,
			pDSwSlackStatement.h1,
			pDSwSlackStatement.h2,
		);
