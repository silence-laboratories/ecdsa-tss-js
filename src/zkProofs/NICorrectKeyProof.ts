...
export default class NICorrectKeyProof {
  ...
  static async _rhoVec(
    ...
    const sidBytes = utils.hexToUint8Array(sid);
    ...
    for (let i = 0; i < NICorrectKeyProof.M2; i++) {
      ...
      let value = await NICorrectKeyProof._maskGeneration(
        BigInt(keyLength),
        new Uint8Array(seedBn)
      );
  ...
  static async prove(
    ...
    const rhoVec = await NICorrectKeyProof._rhoVec(
      publicN,
      keyLength,
      sid,
      pid
    ...
    rhoVec.forEach((rhoValue) => {
      sigmaVec.push(
        NICorrectKeyProof._extractNRoot(paillierPrivateKey, rhoValue)
      );
    });
  ...
  static fromObj(message: INICorrectKeyProof) {
...
export interface INICorrectKeyProof {
  sigma_vec: string[];
}
