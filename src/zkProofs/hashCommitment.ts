...
export async function createCommitment(
    q: secp.Point, dLogProof: DLogProof, blindFactor: bigint, sid: string, pid: string
): Promise<string> {
  ...
  dataToHashList.push(utils.stringToUint8Array(sid));
...
export async function verifyCommitment(
  ...
  const commitmentTest = await createCommitment(q, dLogProof, blindFactor, sid, pid);
