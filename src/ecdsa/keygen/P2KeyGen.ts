...
import { b64ToBigint, bigintTob64, bigintToUint8Array } from "../../utils";
...
export class P2KeyGen {
	...
	toObj(): IP2KeyGen {
		...
		if (this.eph2) {
			// @ts-ignore
			d.eph2 = bigintTob64(this.eph2);
		}
		if (this.commitment1) {
			// @ts-ignore
			d.commitment1 = this.commitment1;
		}
		if (this.commitment2) {
			// @ts-ignore
			d.commitment2 = this.commitment2;
		}
	...
	static fromObj(obj: IP2KeyGen) {
		if (!utils.checkOwnKeys(P2KeyGen.requiredFields, obj)) {
		...
		const keyGenObj = new P2KeyGen(
			sessionId,
			bigintToUint8Array(x2),
			expectedPublicKey,
		);
		keyGenObj._state = obj.state;
		if (obj.eph2) keyGenObj.eph2 = b64ToBigint(obj.eph2);
		if (obj.commitment1) keyGenObj.commitment1 = obj.commitment1;
		if (obj.commitment2) keyGenObj.commitment2 = obj.commitment2;
		return keyGenObj;
	}
	...
	async _processKeyGenMessage1(
		keyGenMessage1: KeyGenMessage1,
	): Promise<KeyGenMessage2> {
		...
		const dLogProof1 = await DLogProof.prove(
			this.x2,
			q2,
			this.sessionId,
			PARTY_ID_2,
		...
		const dLogProof2 = await DLogProof.prove(
			this.eph2,
			e2,
			this.sessionId,
			PARTY_ID_2,
	...
	async _processKeyGenMessage3(
		keyGenMessage3: KeyGenMessage3,
	): Promise<P2KeyShare> {
		...
		const ephPoint = e1.multiply(this.eph2 as bigint);
		...
		const cond6 = await nIKeyCorrectProof.verify(
			paillierPublicKey,
			this.sessionId,
			PARTY_ID_1,
		);
		...
		const p2KeyShare = new P2KeyShare(
			this.x2,
			publicKey,
			cKeyX1,
			paillierPublicKey,
	...
	async processMessage(messageString: string): Promise<IP2KeyGenResult> {
		...
		try {
			if (this._state === P2KeyGenState.PROCESS_KEY_GEN_MSG_1) {
				...
				const keyGenMessage2 = await this._processKeyGenMessage1(
					keyGenMessage1,
				);
				return {
