...
import {
	IPDLwSlackProof,
	PDLwSlackProof,
} from "../../zkProofs/pDLProof/PDLwSlackProof";
import {
	CompositeDLogProof,
	ICompositeDLogProof,
} from "../../zkProofs/wiDLogProof/CompositeDLogProof";
import * as paillier from "paillier-bigint";
import NICorrectKeyProof, {
	INICorrectKeyProof,
} from "../../zkProofs/NICorrectKeyProof";
import {
	IPDLwSlackStatement,
	PDLwSlackStatement,
} from "../../zkProofs/pDLProof/PDLwSlackStatement";
...
export class KeyGenMessage3 {
	...
	static fromObj(message: IKeyGenMessage3) {
		...
		const paillierPublicKey = paillierPublickeyFromStr(
			message.paillier_public_key,
		);
		const nIKeyCorrectProof = NICorrectKeyProof.fromObj(
			message.ni_key_correct_proof,
		);
		const pDSwSlackStatement = PDLwSlackStatement.fromObj(
			message.pdl_w_slack_statement,
		);
		const pDLwSlackProof = PDLwSlackProof.fromObj(message.pdl_w_slack_proof);
		const compositeDLogProof = CompositeDLogProof.fromObj(
			message.composite_dlog_proof,
		);
...
export interface IKeyGenMessage3 {
	phase: string;
	session_id: string;
	q1: string;
	dlog_proof_1: IDLogProof;
	blind_factor_1: string;
	c_key: string;
	paillier_public_key: string;
	ni_key_correct_proof: INICorrectKeyProof;
	pdl_w_slack_statement: IPDLwSlackStatement;
	pdl_w_slack_proof: IPDLwSlackProof;
	composite_dlog_proof: ICompositeDLogProof;
	e1: string;
	dlog_proof_2: IDLogProof;
	blind_factor_2: string;
}
